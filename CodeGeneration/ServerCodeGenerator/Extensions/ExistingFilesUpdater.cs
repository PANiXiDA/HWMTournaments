...
internal static class ExistingFilesUpdater
{
        ...
        GeneratedFile layer)
    {
        ...
        var implementationName = $"{model.Name}s{suffix}";
        ...
        string directory)
    {
        ...
        var lines = File.ReadAllLines(filePath);
        var pattern = $"^\\s*public\\s+virtual\\s+DbSet<\\s*{Regex.Escape(model.Name)}\\s*>\\s+{Regex.Escape(model.Name)}s.*$";
