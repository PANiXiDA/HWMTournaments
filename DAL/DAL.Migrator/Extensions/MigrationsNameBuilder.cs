...
internal static class MigrationsNameBuilder
{
    ...
    private static string Clean(string input)
    {
        ...
        var cleaned = Regex.Replace(sb.ToString(), "_{2,}", "_").TrimEnd('.', ' ');
    ...
    private static string Truncate(string input, int maxLength = MaxNameLength)
    {
        ...
        const int HashLen = 8;
        var hash = BitConverter.ToString(MD5.HashData(Encoding.UTF8.GetBytes(input))).Replace("-", "").Substring(0, HashLen);
